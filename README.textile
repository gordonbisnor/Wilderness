...
<img src='http://www.isnorcreative.com/images/wilderness/dashboard.png' alt='WildernessCMS'/>
...
* Standard gems and plugins: capistrano, will paginate, cucumber, rspec, factory girl, openid, aasm, webrat, acts_as_taggable_on_steroids, attachment_fu, bundle-fu, forgot_password, gravatar-plugin (modified),
...
* Option to Choose From Textile/Markdown/TinyMCE or simple format for text areas
* Extra modules
* Improving form automation, configuration options for index and show views etc.
* Etc.
...
** IF THE APP IS NOT CREATED YET, CREATE IT NOW:
rails -d myapp

* OR DEFAULT TO YOUR FAVORITE DATABASE: 
rails -d mysql myapp

* IF THE DATABASE IS NOT CREATED YET, CREATE IT NOW:
rake db:create

* RUN RAKE INSTALL TO GET SOME REQUIRED FILES INTO YOUR APP:
...
* OMIT FIELDS FOR ADMIN INDEX AND SHOW PAGES (TODO: MAKE THIS A HASH THAT TAKES SHOW OR INDEX AS THE KEY AND FIELDS AS THE VALUE): 
